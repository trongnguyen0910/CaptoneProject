...
class CreateCropGrowthTask extends StatefulWidget {
  @override
  State<CreateCropGrowthTask> createState() => _CreateCropGrowthTaskState();
}

class _CreateCropGrowthTaskState extends State<CreateCropGrowthTask> {
  ...
  int? selectedVarietyStageId;

  Future<void> getVarietyStage() async {
    ...
    if (responseTrans != null) {
      if (responseTrans is List) {
        setState(() {
          datavarietystage =
              responseTrans.map((item) => DataVarietyStage.fromJson(item)).toList();
        });
        Get.find<GardenController>().updateVarietyStageList(datavarietystage);
      }
    }
  }
  ...
  var cropStageList = Get.find<GardenController>().cropStageList;
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    DropdownButtonFormField<int>(
                                      ...
                                      items: cropStageList
                                          .map((DataVarietyStage cropstage) {
                                        return DropdownMenuItem<int>(
                                          value: cropstage.cropVarietyStageId,
                                          child: Text(
                                              cropstage.stageName ?? ''),
                                        );
                                      }).toList(),
                                      onChanged: (int? cropVarietyStageId) {
                                        setState(() {
                                          selectedVarietyStageId = cropVarietyStageId;
                                          print(
                                              'selectedvarietyId: $selectedVarietyStageId');
                                        });
                                      },
                                    )
                                  ],
